...
int main(){
    ...
    if (!storagePosition) printf("б) В %d нет нечетных цифр\n\n",number);
    else printf("б) Минимальная нечетная цифра = %d, ее позиция = %d в числе %d\n\n",responseFromFunction, storagePosition ,number);
...
void searcherDigits(int &number, int &storagePosition, bool comparator(int firstNumber, int secondNumber)){
    ...
    {
        if (comparator == isLessAndOdd) previousDigit = 10;
