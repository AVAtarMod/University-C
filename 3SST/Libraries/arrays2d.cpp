...
#include "arrays2d.hpp"

int* getRangeUser() {
    int* choice = new int[2];

    std::printf("Введите числа А и В диапазона значений псевдо-случайных чисел "
                "[А;В] : ");
...
int* compare(int** arrayA, int** arrayB, int rowsMin, int collumnsMin,
    ...
    for (int row = 0; row < rowsMin; row++) {
        for (int col = 0; col < collumnsMin; col++) {
            if (comparator(arrayA[row][col], arrayB[row][col],
                           arrayResult[iterResult])) {
                iterResult++;
            }
        }
    }
...
void checkWork(int** array, int* size, int* range, const char* text) {
    ...
    std::cin.ignore(INT16_MAX, '\n');
    if (choice == 1) {
        print(array, size[0], size[1], "", range[1]);
        i++;
    } else if (choice != 0) {
