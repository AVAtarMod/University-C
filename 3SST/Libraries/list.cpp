...
IntList1D mergeByAction(const IntList1D listA, const IntList1D listB,
                        int merge_actions(int fromA, int fromB)) {
    IntList1D merged = new IntList1D_element*;
    *merged = new IntList1D_element;
...
bool insert(IntList2D_element* begin, int index, int number) {
    ...
    if (index == 1) {
        IntList2D_element* temp =
            new IntList2D_element(begin, number, begin->next);
        begin->next = temp;
...
void doActionOnIndexes(IntList1D list, const IntList1D indexes,
    ...
    while (current != nullptr && indexes_ptr != nullptr) {
        if (indexes_ptr->data == index) {
            index_action(&current, index_action_data);
            indexes_ptr = indexes_ptr->next;
        }
        current = current->next;
        ++index;
...
IntList1D GEN_USERINPUT_newline(const uint& buffer_size, bool& printInfo) {
    if (printInfo)
        std::cout << "[INFO] Entered data must be lesser than "
                  << buffer_size + 1 << "\n";
    std::cout << "Enter list: ";
...
bool PUSH_Before(IntList1D_element* current, int number) {
    if (current != nullptr) {
        IntList1D_element* temp =
            new IntList1D_element(current->data, current->next);
        current->data = number;
        current->next = temp;
