...
int main()
{
    ...
    if (undefiendRise)
        printf("Функция y=x^3 + sin(x) ведет себя по-разному на промежутке [-1, 1]\n\n");
...
double CompareTo(double firstNumber, double secondNumber)
{
    if (firstNumber - secondNumber <= __FLT_EPSILON__)
        return -1;
    else if (fabs(firstNumber - secondNumber) <= __FLT_EPSILON__ * fabs(firstNumber))
        return 0;
    else
        return 1;
}
