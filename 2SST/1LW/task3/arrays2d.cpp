...
void print(int** array, int rows, int collumns, const char* text, int maxElement, int* highlight, std::string highlightColor)
{
    ...
    for (int row = 0, printRow = row; row < rows; row++) {
        if (array[row]) {
            printf("%d [ ", printRow);
            for (int coll = 0; coll < collumns; coll++) {
                bool needHighlight = (row == highlight[0] && coll == highlight[1]) ? true : false;
                if (needHighlight)
                    printf("%s%*d %s,", highlightColor.c_str(), maxElement, array[row][coll], COLORS::reset.c_str());
                else
                    printf("%*d ,", maxElement, array[row][coll]);
            }
            printf("\b ]\n");
            ++printRow;
        }
...
void print(int** array, int rows, int collumns, const char* text, int maxElement, int* highlight, int highlightSize, std::string highlightColor)
{
    ...
    for (int row = 0, i = 0, printRow = row; row < rows; row++) {
        if (array[row]) {
            printf("%d [ ", printRow);
            for (int coll = 0; coll < collumns; coll++) {
                if (highlight[i] == -1 && highlight[i + 1] == i - 1 && i < highlightSize - 1)
                    i += 2;
                bool needHighlight = (row == highlight[i] && coll == highlight[i + 1]) ? true : false;
                if (row >= highlight[i] && coll >= highlight[i + 1] && i < highlightSize - 1)
                    i += 2;
                if (needHighlight)
                    printf("%s%*d %s,", highlightColor.c_str(), maxElement, array[row][coll], COLORS::reset.c_str());
                else
...
void checkWork(int** array, int* size, int* range, const char* text)
{
    ...
    std::cin.ignore(INT16_MAX, '\n');
    if (choice == 1) {
        array2d::print(array, size[0], size[1], "", range[1], nullptr, COLORS::fgRed);
        i++;
    } else if (choice != 0) {
