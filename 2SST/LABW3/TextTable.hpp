...
public:
    ...
    unsigned correctDistance(std::string string_to_correct) const
    {
        return static_cast<unsigned>(string_to_correct.size()) - static_cast<unsigned>(glyphLength(string_to_correct));
    };
...
inline std::ostream &operator<<(std::ostream &stream, TextTable const &table)
{
    ...
    for (auto rowIterator = table.rows().begin(); rowIterator != table.rows().end(); ++rowIterator)
    {
        ...
        for (unsigned i = 0; i < row.size(); ++i)
        {
            ...
            stream << std::setw(static_cast<int>(table.width(i) + table.correctDistance(row[i]))) << alignment << row[i];
            stream << table.vertical();
        }
        stream << "\n";
        if (table.has_ruler())
